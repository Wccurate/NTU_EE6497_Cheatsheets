...
\item $p(x | y) = \frac{p(x,y)}{p(y)} = \frac{p(y | x) p(x)}{\int p(y \mid x)p(x)\,dx}$.
...
% $\text{Cat} (x \mid \theta_1, \dots, \theta_K)$ with parameters 
$\theta_k \geq 0, \sum_{k=1}^{K} \theta_k = 1$;
...
&= \frac{\exp(-\frac{1}{2}q)}
{(2\pi)^{K/2}|\boldsymbol{\Sigma}|^{1/2}},\\
q&=(\mathbf{x}-\boldsymbol{\mu})^\top
...
For a general random vector with $\mathbb{E}[\mathbf{X}]=\boldsymbol{\mu}$ and $\text{cov}(\mathbf{X})=\boldsymbol{\Sigma}$,
...
For scalar projection $Y=\mathbf{a}^\top\mathbf{X}$:
$\mathbb{E}[Y]=\mathbf{a}^\top\boldsymbol{\mu}, \quad
\text{var}(Y)=\mathbf{a}^\top\boldsymbol{\Sigma}\mathbf{a}$.
...
$L^{(0)} = \frac{1}{n} \sum_i \log \sum_k \pi^{(0)}(k) \mathcal{N} (\mathbf{x}_i \mid \boldsymbol{\mu}_k^{(0)}, \boldsymbol{\Sigma}_k^{(0)})$. 
\textbf{Repeat:}

1. \textbf{E-step:} Compute responsibilities:
    $r_{ik}^{(m)} =
    \frac{\pi_k^{(m)} \mathcal{N}(\mathbf{x}_i \mid \boldsymbol{\mu}_k^{(m)}, \boldsymbol{\Sigma}_k^{(m)})}
    {\sum_{j} \pi_j^{(m)} \mathcal{N}(\mathbf{x}_i \mid \boldsymbol{\mu}_j^{(m)}, \boldsymbol{\Sigma}_j^{(m)})}$,  
...
2. \textbf{M-step:} Update parameters:
    ...
    $\boldsymbol{\Sigma}_k^{(m+1)} =
    \frac{1}{n_k^{(m)}} \sum_i r_{ik}^{(m)}
    (\mathbf{x}_i - \boldsymbol{\mu}_k^{(m+1)})
    (\mathbf{x}_i - \boldsymbol{\mu}_k^{(m+1)})^\top$. 

3. \textbf{Compute:}
    $L^{(m+1)} = \frac{1}{n} \sum_i \log \sum_k
    \pi_k^{(m+1)} \mathcal{N}(\mathbf{x}_i \mid
    \boldsymbol{\mu}_k^{(m+1)}, \boldsymbol{\Sigma}_k^{(m+1)})$. 
...
GMM with $\Sigma_k = \sigma^2 I$ and $\pi(k) = 1/K$ are fixed. Only $\mu_k$ are inferred. \quad
...
\bulletPoint{Basic Distributions} \quad

\textbf{Bernoulli} $X\sim\mathrm{Bern}(\theta)$, $x\in\{0,1\}$:
$p(x)=\theta^x(1-\theta)^{1-x}$,
$\mathbb{E}X=\theta$, $\mathrm{var}(X)=\theta(1-\theta)$. \quad
\textbf{Categorical} $X\sim\mathrm{Cat}(\boldsymbol{\theta})$:
$p(X=k)=\theta_k$, $\sum_k\theta_k=1$.
One-hot $\mathbf{Y}$: $\mathbb{E}Y_k=\theta_k$,
$\mathrm{var}(Y_k)=\theta_k(1-\theta_k)$,
$\mathrm{cov}(Y_j,Y_k)=-\theta_j\theta_k$. \quad
\textbf{Uniform} $X\sim\mathrm{Unif}(a,b)$:
$p(x)=\frac{1}{b-a}\mathbbm{1}\{a\le x\le b\}$,
$\mathbb{E}X=\frac{a+b}{2}$,
$\mathrm{var}(X)=\frac{(b-a)^2}{12}$. \quad
\textbf{Exponential} $X\sim\mathrm{Exp}(\lambda)$:
$p(x)=\lambda e^{-\lambda x}\mathbbm{1}\{x\ge0\}$,
$\mathbb{E}X=\frac{1}{\lambda}$,
$\mathrm{var}(X)=\frac{1}{\lambda^2}$. \quad
\textbf{Binomial} $X\sim\mathrm{Bin}(n,\theta)$:
$p(x)=\binom{n}{x}\theta^x(1-\theta)^{n-x}$,
$\mathbb{E}X=n\theta$,
$\mathrm{var}(X)=n\theta(1-\theta)$. \quad
\textbf{Beta} $X\sim\mathrm{Beta}(a,b)$:
$p(x)=\frac{\Gamma(a+b)}{\Gamma(a)\Gamma(b)}
x^{a-1}(1-x)^{b-1}$, $0<x<1$,
$\mathbb{E}X=\frac{a}{a+b}$,
$\mathrm{var}(X)=\frac{ab}{(a+b)^2(a+b+1)}$. \quad
\textbf{Gaussian} $X\sim\mathcal{N}(\mu,\sigma^2)$:
$p(x)=\frac{1}{\sqrt{2\pi\sigma^2}}
e^{-\frac{(x-\mu)^2}{2\sigma^2}}$,
$\mathbb{E}X=\mu$, $\mathrm{var}(X)=\sigma^2$. \quad
\textbf{Multivariate Gaussian} $\mathbf{X}\sim\mathcal{N}(\boldsymbol{\mu},\boldsymbol{\Sigma})$:
$p(\mathbf{x})=\frac{\exp(-q/2)}
{(2\pi)^{K/2}|\boldsymbol{\Sigma}|^{1/2}}$,
$q=(\mathbf{x}-\boldsymbol{\mu})^\top
\boldsymbol{\Sigma}^{-1}(\mathbf{x}-\boldsymbol{\mu})$,
...
\bulletPoint{Derivative and Integral Basics} \quad
...
$(\tanh x)'=1-\tanh^2x$,
$(\mathrm{ReLU}(x))'=\mathbbm{1}\{x>0\}$. \quad
\textbf{Common partials:}
$\frac{\partial}{\partial x_i}\sum_j x_j=1$,
$\frac{\partial}{\partial x_i}\sum_j x_j^2=2x_i$,
$\nabla_{\mathbf{x}}\|\mathbf{x}\|^2=2\mathbf{x}$,
$\nabla_{\mathbf{x}}\|\mathbf{A}\mathbf{x}-\mathbf{b}\|^2
=2\mathbf{A}^\top(\mathbf{A}\mathbf{x}-\mathbf{b})$. \quad
\textbf{Integrals:}
...
$\int_a^b(f+g)dx=\int_a^b fdx+\int_a^b gdx$.
...
# containers / model
model = nn.Sequential(
    nn.Linear(in_features=D, out_features=H, bias=True),
    nn.ReLU(),                 # also Sigmoid(), Tanh(), LeakyReLU(a)
    nn.Dropout(p=0.5),         # active only in model.train()
    nn.Linear(H, C)
)
...
nn.Embedding(num_embeddings=V, embedding_dim=E)
nn.LSTM(input_size=D, hidden_size=H, num_layers=1,
        batch_first=True, bidirectional=False, dropout=0)

# losses
nn.CrossEntropyLoss(weight=None)  # multiclass; input logits (N,C), target class id
nn.BCEWithLogitsLoss(pos_weight=None) # binary/multilabel; logits
nn.MSELoss(reduction='mean')      # regression
nn.L1Loss(reduction='mean')       # MAE
nn.SmoothL1Loss(beta=1.0)         # Huber-like
nn.NLLLoss()                      # after LogSoftmax
...
model.eval()  # inference; affects Dropout/BatchNorm
\end{lstlisting}
